...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        # self.OFFICIAL_LSTM = nn.LSTM(input_size=emb_size, hidden_size=n_hidden)
        self.LSTM = MyLSTM(emb_size, n_hidden, n_layer)
    ...
    def forward(self, X):
        ...
        # official_outputs, (_, _) = self.OFFICIAL_LSTM(X)
        outputs = self.LSTM(X)
    ...
    for epoch in range(all_epoch):
        ...
        with torch.no_grad():
            ...
            for valid_batch, valid_target in zip(all_valid_batch, all_valid_target):
                ...
                total_loss = total_loss+valid_loss.item()
